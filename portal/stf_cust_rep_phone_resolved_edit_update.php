...
include_once("signon/pdo-connect.php");
    ...
    if ($status == "") {
        ...
        try {
            // We Will prepare SQL Query
            $str_query = "  UPDATE tbl_cust_rep_contact
                            SET cust_name=:cust_name, cust_email=:cust_email, cust_phone=:cust_phone, cust_country=:cust_country, cust_note=:cust_note, status=8 
                            WHERE id = :id;";
            $str_stmt = $r_Db->prepare($str_query);
            // bind paramenters, Named paramenters alaways start with colon(:)
            $str_stmt->bindParam(':id', $i_uID);
    ...
    header("location:stf_cust_rep_phone_resolved_edit.php?usr=$i_uID&status=$status"); 
?>