...
include_once("signon/pdo-connect.php");
    ...
    if ($status == "") {
        ...
        try {
            // We Will prepare SQL Query
            $str_query = "  UPDATE tbl_consult_contact
                            SET consult_name=:consult_name, consult_company=:consult_company, consult_country=:consult_country, consult_email=:consult_email, consult_phone=:consult_phone, consult_note=:consult_note, consult_outcome=:consult_outcome , status=9 
                            WHERE id = :id;";
            $str_stmt = $r_Db->prepare($str_query);
            // bind paramenters, Named paramenters alaways start with colon(:)
            $str_stmt->bindParam(':id', $i_uID);
    ...
    header("location:stf_cust_consult_phone_unresolved_edit.php?usr=$i_uID&status=$status"); 
?>